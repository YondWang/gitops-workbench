...
class FeaturePackageTest(unittest.TestCase):
    ...
    def test_collision_allocation_and_ui_ci_contract(self):
        ...
        self.assertEqual(ci.count('tags: ["simos-feature-build"]'), 2)
        self.assertEqual(ci.count('tags: ["gitops-feature-publisher"]'), 2)
        self.assertNotIn("GIT_STRATEGY: none", ci)
        self.assertEqual(ci.count("GIT_STRATEGY: fetch"), 4)
        self.assertEqual(ci.count('image: "$GITOPS_FEATURE_BUILD_IMAGE"'), 3)
        self.assertNotIn("apk add", ci)
        self.assertNotIn("stage: upload", ci)
        self.assertNotIn("SIMOS_OTA", ci)
        self.assertNotIn("feature_prepare", ci)
        self.assertNotIn("- feature-source/", ci)
        self.assertIn("job: feature_context_validate", ci)
        build = (server.ROOT.parent / ".gitlab" / "scripts" / "feature-package-build.sh").read_text(encoding="utf-8")
        self.assertIn('ci/resident/build-performance.env', build)
        self.assertIn('SIMOS_FEATURE_BUILD_JOBS', build)
        self.assertIn('container-build-resident.sh', build)
        self.assertIn('cp -a "$source_dir/resident-packages" "$output_dir/resident-packages"', build)
        registry_publisher = (server.ROOT.parent / ".gitlab" / "scripts" / "feature-package-publish-registry.sh").read_text(encoding="utf-8")
        nextcloud_publisher = (server.ROOT.parent / ".gitlab" / "scripts" / "feature-package-publish-nextcloud.sh").read_text(encoding="utf-8")
        self.assertIn('out.rglob("*")', registry_publisher)
        self.assertIn('item.get("path")', nextcloud_publisher)
        prepare = (server.ROOT.parent / ".gitlab" / "scripts" / "feature-package-prepare.sh").read_text(encoding="utf-8")
